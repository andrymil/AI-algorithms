...
def track_accuracy_with_process_pool(start_epoch, end_epoch, num_samples, run_times=3, learning_rate=0.01, batch_size=8, layers=[64, 128, 64, 10]):
    def run_track_accuracy(_):
        return track_accuracy_over_epochs(start_epoch, end_epoch, num_samples, learning_rate, batch_size, layers)

    with ProcessPoolExecutor() as executor:
        results = list(executor.map(run_track_accuracy, range(run_times)))

    train_accuracies = [result[1] for result in results]
    val_accuracies = [result[2] for result in results]

    # Plot the accuracies
    plt.boxplot(val_accuracies, positions=np.linspace(start_epoch, end_epoch, num_samples, dtype=int), widths=0.5)
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.title('Validation Accuracy vs Epochs')
    plt.show()
...
# Example usage:
# track_accuracy_with_process_pool(start_epoch=1, end_epoch=100, num_samples=10, run_times=3)
...
if __name__ == "__main__":
    ...
    # print(results)
    track_accuracy_over_epochs_with_boxplot(start_epoch=1, end_epoch=100, num_samples=10, num_runs=3)
